...
const NavLink = ({ children, to, isActive, badge }) => (
  ...
  >
    {children}
    {badge && (
      <span className="ml-1 px-1.5 py-0.5 text-xs bg-yellow-400 text-gray-900 rounded-full">
        {badge}
      </span>
    )}
    {isActive && (
      <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-white rounded-full" />
    )}
  </Link>
);
...
const MobileNavLink = ({ children, to, isActive, badge, onClick }) => (
  ...
  >
    <span>{children}</span>
    {badge && (
      <span className="px-1.5 py-0.5 text-xs bg-yellow-400 text-gray-900 rounded-full">
        {badge}
      </span>
    )}
  </Link>
